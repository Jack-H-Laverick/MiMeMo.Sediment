...
ggplot(grid) + geom_sf(aes(fill = log(August+1)), size = 0.001, colour = NA) +
...
#saveRDS(weighted, "./Objects/Seasonal_Stress.rds")                                 # Save

layers <- map(as.list(month.name), get_raster) %>% 
  brick()
